...
if jsonresponse_moving_points.status=="true"
    current_id_moving_points = str2num(jsonresponse_moving_points.id); %#ok<*ST2NM>  
end
...
while true
    ...
    arrayString = jsonresponse_moving_points.new_path;  
        % Split the string into individual coordinate strings
        coordStrings = strsplit(arrayString, ') ');
        coordStrings = coordStrings(1:end-1); % Remove the last empty element
        
        % Initialize coordinate vectors
        x_moving_path = zeros(1, length(coordStrings));
        y_moving_path = zeros(1, length(coordStrings));
        z_moving_path = zeros(1, length(coordStrings));
        ...
        for i = 1:length(coordStrings)
            ...
            x_moving_path(i) = coords(1);
            y_moving_path(i) = coords(2);
            z_moving_path(i) = coords(3);
        end
        
        % Plotting
        plot3(x_moving_path, y_moving_path, z_moving_path, 'o-');
        xlabel('X');
        ylabel('Y');
        zlabel('Z');
        title('3D Plot of Coordinates');
        % Adjusting the axes limits
        xlim([min(x_moving_path) max(x_moving_path)]);
        ylim([min(y_moving_path) max(y_moving_path)]);
        zlim([min(z_moving_path) max(z_moving_path)]); 
        % Optional: Adjusting the view angle
