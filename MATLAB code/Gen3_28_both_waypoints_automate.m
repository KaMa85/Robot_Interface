...
while true
    ...
    while previouslyknownid < currentvideoid
        ...
        points0 = [path1_resampled; path2_resampled; path3_resampled; path4_resampled];
        xx0 = points0(:, 1);
        yy0 = points0(:, 2);
        zz0 = points0(:, 3);
        
        % Length of the concatenated path
        m = length(xx0); % This should be 4 * minLength
        
% Generate the cosine-based parameterization for the entire path
s_uniform = linspace(0, 1, m)'; % Parameterization length matches m
s_uniform = 0.5 - 0.5*cos(pi * s_uniform); % Cosine-based parameterization
...
zz_interp = interp1(linspace(0, 1, m), zz0, s_uniform, 'linear');
        ...
        % Reduce the number of points to match the original number of points
        final_indices = linspace(1, m, minLength)'; % Generate minLength evenly spaced indices
        xx_final = interp1(1:m, xx_interp, final_indices, 'linear');
        yy_final = interp1(1:m, yy_interp, final_indices, 'linear');
        zz_final = interp1(1:m, zz_interp, final_indices, 'linear');
        
        % Plot to visualize the results (optional)
        figure;
        plot3(xx_final, yy_final, zz_final, '-o');
        xlabel('X');
        ylabel('Y');
        zlabel('Z');
        title('Path with Zero Velocity at Start of Path1 and End of Path4');
        grid on;
        
        % Assign the final interpolated values back to the original variables
        xx = xx_final;
        yy = yy_final;
        zz = zz_final;
        points = [xx, yy, zz];
