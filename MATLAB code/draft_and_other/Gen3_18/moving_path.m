function [x_moving_path, y_moving_path, z_moving_path] = moving_path(ax,ay,az,constr, startpoint, endpoint, xx, yy, zz, modified_path)
...
eps=0.075;
x_m_p = x_m_p(1:length(xx),1);
y_m_p = y_m_p(1:length(xx),1);
z_m_p = z_m_p(1:length(xx),1);
% Detecting changes
changedX = (xx - x_m_p > eps) | (xx - x_m_p < -eps);
changedY = (yy - y_m_p > eps) | (yy - y_m_p < -eps);
changedZ = (zz - z_m_p > eps) | (zz - z_m_p < -eps);
