...
while true
    ...
    while previouslyknownid < currentvideoid
        ...
        if jsonresponse.status=="true"
            ...
            if gripper_state==1
                if isempty(auto_struct.first_gripper_action)
        ...
        while 1
            if (last_id_moving_points==current_id_moving_points)
                ...
                if run_moving_path.State =="queued" || run_moving_path.State =="finished"
                    ...
                    webwrite(url_to_HL_update,'json_points',jsonPoints_moving);
                    pause(0.35);                    
                    % Updating the database id for moving point (max 1s)
                    tic
                    while true
                        apiresponse_moving_points = webread(url_from_HL_moving_points);
                        jsonresponse_moving_points=jsondecode(apiresponse_moving_points);
            ...
            if isOk
                if max(abs(wrapTo360(qs_deg(1,:))-actuatorFb.position)) < 1
                    ...
                    newArray = [qs_deg.'; vel.'; acc.'; timestamp];
                    if isempty(auto_struct.first_motion)
                        auto_struct.first_motion=newArray;
                    else
                        auto_struct.second_motion=newArray;
                    end
                    break;
                end 
            else
