    ...
    %% Compute Joint Angle Based on Inverse Kinematics of the Path and Animate
        ...
        for i = 1:1:numWaypoints
            ...
            if (last_id_moving_points==current_id_moving_points)
                ...
                apiresponse_moving_points = webread(url_from_HL_moving_points);
                jsonresponse_moving_points=jsondecode(apiresponse_moving_points);
                run_moving_path= parfeval(backgroundPool, @moving_path, 3, constr, points(i,:), points(end,:), xx , yy , zz, jsonresponse_moving_points.new_path);
                if run_moving_path.State =="queued" || run_moving_path.State =="finished"
                    ...
                    hold on;
                    plot3(points(:,1),points(:,2),points(:,3),'-g','LineWidth',2);
                    plot3(x_moving_path,y_moving_path,z_moving_path,'-r','LineWidth',2); 
                    axis ([-0.7 0.8 -0.8 0.6 0 1]);
                    view([45,0]);
                    grid('minor');
                    drawnow;
