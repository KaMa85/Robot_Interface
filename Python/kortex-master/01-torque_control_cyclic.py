...
import os
import sys
#pip install "C:\!\Dessertation\Technical\robot_codes(Matlab_python)\Python\kortex-master\kortex_api-2.6.0.post3-py3-none-any.whl"
...
class TorqueExample:
    def __init__(self, router, router_real_time):
        ...
        self.base_cyclic_high_level=BaseCyclicClient(router)
    ...
    def MoveToHomePosition(self):
        ...
        feedback = self.base_cyclic_high_level.RefreshFeedback()
    
        cartesian_pose = action.reach_pose.target_pose
        cartesian_pose.x = feedback.base.tool_pose_x+0.0          # (meters)
        cartesian_pose.y = feedback.base.tool_pose_y - 0.0    # (meters)
        cartesian_pose.z = feedback.base.tool_pose_z - 0.0    # (meters)
        cartesian_pose.theta_x = feedback.base.tool_pose_theta_x+0 # (degrees)
        cartesian_pose.theta_y = feedback.base.tool_pose_theta_y+0 # (degrees)
        cartesian_pose.theta_z = feedback.base.tool_pose_theta_z+0 # (degrees)
...
def main():
    ...
    parser.add_argument("--duration", type=int, help="example duration, in seconds (0 means infinite)", default=60)
